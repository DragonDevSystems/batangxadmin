...
<script type="text/javascript">
	...
	{
		$.get('{{URL::Route('customerList')}}', function(data)
		{
			$('.tbl-overlay').remove();
			if(data.length != 0)
			{
		...
		$.get('{{URL::Route('uaal')}}',{ uid: uid}, function(data)
		{
			if(data.length != 0)
			{
		        ...
		        $appendItems.append($('<select />' , { 'id':'module' ,'class':'form-control select2' ,'name':'module', 'disabled': true}).append(
										...
										'<option value="1">Block User</option>'));
		       	$("#module").val(data.uinfo.blocked);
	...
	function defaultDisplay()
	{
		...
		$('#div_user-entry').append(
				$('<div />' , { 'class' : 'box-header with-border'}).append(
					$('<h3 />' , { 'class' : 'box-title' , 'text' : 'Customer List'}),
					$('<div />' , { 'class' : 'box-tools pull-right'}).append(
    ...
    function updateRecord(uid)
    {
    	...
    	$('#btnYes').click(function() {
    		...
    		$.post('{{URL::Route('updateCustomer')}}',{ _token: $_token, role: $role, uid: uid}, function(data)
			{
				if(data.status == "success")
				{
					adminUserList();
